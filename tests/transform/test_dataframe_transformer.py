...
from src.transform.dataframe_transformer import add_columns_with_defaults, apply_date_format_transformation, compare_tgt_to_src_columns, src_to_target_map
...
def test_apply_date_format_transformation(spark_session):
    ...
    target_columns = {"date_column": {"data_type":"date", "transformation": "date_format", "date_format": "yyyy-MM-dd"}}
    result_df = apply_date_format_transformation(df, target_columns)

    assert result_df.select("date_column").distinct().collect()[0]["date_column"] == "2022-01-01"
...
def test_src_to_target_map(spark_session):
    ...
    target_map = {"name": {"data_type": "string"}, "age": {"data_type": "integer"}, "new_col": {"data_type": "string", "default": "default_value"}}
